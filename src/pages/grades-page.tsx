        ...
        {isDescriptive ? (
          ...
          <TableFrame className="table-frame--borderless">
            <table className="data-table grade-results-table">
              ...
              <tbody>{computedRows.map((item, index) => <tr className={item.result.transmutedGrade < 75 ? "row-danger" : undefined} key={item.learner.id}><td>{index + 1}</td><td><strong>{learnerName(item.learner)}</strong><small>LRN {item.learner.lrn}</small></td><td>{item.result.wwWeighted.toFixed(2)}</td><td>{item.result.ptWeighted.toFixed(2)}</td><td>{item.result.examWeighted.toFixed(2)}</td><td>{item.result.initialGrade.toFixed(2)}</td><td><Badge tone={gradeTone(item.result.transmutedGrade)}>{item.result.transmutedGrade}</Badge></td><td><strong>{item.result.descriptor.label}</strong><small>{item.result.descriptor.Filipino}</small></td><td><Button size="sm" variant="quiet" onClick={() => navigate(`/forms?form=sf9&learner=${item.learner.id}`)}><FileText size={15} /> SF9</Button></td></tr>)}</tbody>
            </table>
          </TableFrame>
        )}
      </Panel>
